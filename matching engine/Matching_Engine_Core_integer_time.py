...
def match(argument):
    ...
    #If the order is of type "New order bid" or "New order ask"
    ...
    #If the order type is cancel order 
        ...
        if (order is not None):
            ...
            if (order.status == "executed"):
                ...
                ret_list.append({"Response" : 5, "Order ID" : return_order_id,  "Reason" : "The order has already been executed, hence cannot be cancelled"})
        ...
        #If order is not found
            ...
            ret_list.append({"Response" : 5, "Order ID" : return_order_id, "Reason" : "Order with given ID does not exist"})
...
def give_top_n(required_stock, n):
    ...
    #List of the popped orders, so that they can be pushed back after they are recorded
    push_list = list()
    ...
    no_of_buy_nulls : number of elements to be appended as NULL
...
print ("Two buy orders at price 100 : expected responses 0\n")

print (match({"User ID" : "Vishal", "Order type" : 0, "Order ID" : "1", "Security ID" : "amzn", "Order Qty" : 2, "Price" : 100, "Time stamp" : 3, "Return Order ID" : "1"}))

print (match({"User ID" : "Vishal", "Order type" : 0, "Order ID" : "2", "Security ID" : "amzn", "Order Qty" : 6, "Price" : 100, "Time stamp" : 3, "Return Order ID" : "2"}))
print ("----------------------------------------------------------------------------------------------")

print ("One sell order at price 100: expected response 0 and then full execution of two orders by matching \n")

print (match({"User ID" : "Vishal", "Order type" : 1, "Order ID" : "3", "Security ID" : "amzn", "Order Qty" : 6, "Price" : 99, "Time stamp" : 3, "Return Order ID" : "3"}))
print ("----------------------------------------------------------------------------------------------")

print ("Five different orders for google at different prices and quantities, all expected responses 0\n")

print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "4", "Security ID" : "google", "Order Qty" : 13, "Price" : 1000, "Time stamp" : 3, "Return Order ID" : "4"}))

print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "5", "Security ID" : "google", "Order Qty" : 10, "Price" : 100, "Time stamp" : 3, "Return Order ID" : "5"}))


print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "6", "Security ID" : "google", "Order Qty" : 20, "Price" : 200, "Time stamp" : 3, "Return Order ID" : "6"}))


print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "7", "Security ID" : "google", "Order Qty" : 15, "Price" : 300, "Time stamp" : 3, "Return Order ID" : "7"}))


print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "8", "Security ID" : "google", "Order Qty" : 20, "Price" : 400, "Time stamp" : 3, "Return Order ID" : "8"}))
print ("----------------------------------------------------------------------------------------------")

print ("Give top 3 orders for google \n")
print (give_top_n("google", 3))

print ("---------------------------------------------------------------------------------------------")

print ("Give top 3 orders for amazon, as there is only one such order (the first one), we will get only one order as output\n")
print (give_top_n("amzn", 3))
print ("---------------------------------------------------------------------------------------------")

print ("We will now amend the order for amzn, changing the price to 120, thus we expect response 1\n")

print (match({"User ID" : "Vishal", "Order type" : 2, "Order ID" : "1", "Security ID" : "amzn", "Order Qty" : 2, "Price" : 120, "Time stamp" : 3, "Return Order ID" : "1"}))
print ("---------------------------------------------------------------------------------------------")
print ("Now the value of price in top 3 orders of amzn should be changed \n")
print (give_top_n("amzn", 3))

print ("----------------------------------------------------------------------------------------------")

print ("We will place one sell order of price 1000 of higher quantity than top buy order, hence buy order will fully match and sell order will partially match (responses 6 and 7)\n")

print (match({"User ID" : "rushi", "Order type" : 1, "Order ID" : "9", "Security ID" : "google", "Order Qty" : 15, "Price" : 1000, "Time stamp" : 3, "Return Order ID" : "9"}))
print ("----------------------------------------------------------------------------------------------")

print ("3 more orders of higher price which will not get matched, but are of sell type, hence will be seen in top orders, expected response 0\n")

print (match({"User ID" : "rushi", "Order type" : 1, "Order ID" : "10", "Security ID" : "google", "Order Qty" : 26, "Price" : 2000, "Time stamp" : 3, "Return Order ID" : "10"}))

print (match({"User ID" : "rushi", "Order type" : 1, "Order ID" : "11", "Security ID" : "google", "Order Qty" : 5, "Price" : 3000, "Time stamp" : 3, "Return Order ID" : "11"}))

print (match({"User ID" : "rushi", "Order type" : 1, "Order ID" : "12", "Security ID" : "google", "Order Qty" : 7, "Price" : 4000, "Time stamp" : 3, "Return Order ID" : "12"}))
print ("----------------------------------------------------------------------------------------------")

print ("Give the top 6 orders of google expected 4 buy and 4 sell orders\n")

print (give_top_n("google", 6))

print ("----------------------------------------------------------------------------------------------")

print ("We will now amend one sell order and reduce its price so that it gets executed\n")

print (match({"User ID" : "rushi", "Order type" : 2, "Order ID" : "9", "Security ID" : "google", "Order Qty" : 7, "Price" : 400, "Time stamp" : 3, "Return Order ID" : "9"}))

print ("----------------------------------------------------------------------------------------------")


print ("We will now cancel one order, expected response 2\n")

print (match({"User ID" : "rushi", "Order type" : 3, "Order ID" : "12", "Security ID" : "google", "Order Qty" : 7, "Price" : 4000, "Time stamp" : 3, "Return Order ID" : "12"}))

print ("----------------------------------------------------------------------------------------------") 

print ("We will try to cancel an already executed order, and it should get rejected, hence response should be 5\n")

print (match({"User ID" : "Vishal", "Order type" : 3, "Order ID" : "2", "Security ID" : "amzn", "Order Qty" : 6, "Price" : 100, "Time stamp" : 3, "Return Order ID" : "2"}))

print ("----------------------------------------------------------------------------------------------") 

print ("We will try to amend an already cancelled order, and it should get rejected, hence response should be 4\n")

print (match({"User ID" : "Vishal", "Order type" : 2, "Order ID" : "12", "Security ID" : "amzn", "Order Qty" : 6, "Price" : 100, "Time stamp" : 3, "Return Order ID" : "12"}))

print ("----------------------------------------------------------------------------------------------") 

print ("We will not try to demonstrate the importance of timestamp, orders with order IDs 13, 14 and 15,\n13 with lesser timestamp than 14 so ideally 13 should get matched with 15 with identical price\n")

print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "13", "Security ID" : "apple", "Order Qty" : 7, "Price" : 4000, "Time stamp" : 2, "Return Order ID" : "13"}))

print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "14", "Security ID" : "apple", "Order Qty" : 7, "Price" : 4000, "Time stamp" : 3, "Return Order ID" : "14"}))

print (match({"User ID" : "rushi", "Order type" : 1, "Order ID" : "15", "Security ID" : "apple", "Order Qty" : 7, "Price" : 4000, "Time stamp" : 3, "Return Order ID" : "15"}))

print ("----------------------------------------------------------------------------------------------") 

print ("We will now try to demonstrate that one order can match with multiple orders. We will place 3 orders, the third order will match with the first two\n")

print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "16", "Security ID" : "apple", "Order Qty" : 3, "Price" : 4000, "Time stamp" : 2, "Return Order ID" : "16"}))

print (match({"User ID" : "rushi", "Order type" : 0, "Order ID" : "17", "Security ID" : "apple", "Order Qty" : 4, "Price" : 4000, "Time stamp" : 2, "Return Order ID" : "17"}))

print (match({"User ID" : "rushi", "Order type" : 1, "Order ID" : "18", "Security ID" : "apple", "Order Qty" : 7, "Price" : 4000, "Time stamp" : 2, "Return Order ID" : "18"}))

print ("----------------------------------------------------------------------------------------------")

print ("We will now print the entire log of all orders given till date\n\n")
...
for order in all_orders:
    ...
    if (price < 0):
        ...
        typeoforder = "Sell"
    print ("User ID : ", order.user_id, "| Type : ", typeoforder, "| Stock ID : ", order.stock_id, "| Quantity : ", order.quantity, "| Price : ", price, "| Timestamp : ", order.timestamp, "| Status : ", order.status, "\n\n")
